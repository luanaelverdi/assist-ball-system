...
export const getPlayersByIdDt = async (id_dt: number): Promise<Array<DtPlayers> | null> => {
    const query: Array<DtPlayers> = await Postgres.query()`SELECT id_player FROM dt_players WHERE id_dt = ${id_dt};`;
    return query;
};
